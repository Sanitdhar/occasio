...
| #   | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| D40 | **Every PR waits for CodeRabbit, and every finding gets a response**: fix it, reply why it is not needed, or escalate a decision to the maintainer. Never merge with findings unanswered, and verify a finding before accepting it                                                                                                                                                                                                               |
| D41 | **Stacked PRs** for dependent work — branch off the PR below and target it as base, so each diff shows only its own change. Rebase the stack with `--force-with-lease` when a lower PR changes                                                                                                                                                                                                                                                   |
| D42 | **Claude Code reviews a PR only as CodeRabbit's fallback**, triggered solely by CodeRabbit's own "Review limit reached" comment (D40's exhausted-budget case) — never a second reviewer running alongside it. Installed as the official [Claude GitHub app](https://github.com/apps/claude); authenticated with a `CLAUDE_CODE_OAUTH_TOKEN` repo secret                                                                                          |
...
| D43 | **Component tests render through react-native-web in jsdom**, not through `jest-expo`. The Expo preset pulls `react-server-dom-webpack`, whose peer range excludes the React that SDK 57 pins, so it installs only with `--legacy-peer-deps` — which would silence every genuine peer conflict in the repo from then on. The web path needs no such concession and is the platform D30 ships first. Native rendering stays the visual gate's job |
